...
std::string ValueCodec::encodeEdgeAdjacency(VertexId src_id, VertexId dst_id, uint64_t seq, EdgeLabelId label_id) {
    ...
    size_t off = 0;
    for (int i = 7; i >= 0; --i) result[off++] = static_cast<char>((src_id >> (i * 8)) & 0xFF);
    for (int i = 7; i >= 0; --i) result[off++] = static_cast<char>((dst_id >> (i * 8)) & 0xFF);
    for (int i = 7; i >= 0; --i) result[off++] = static_cast<char>((seq >> (i * 8)) & 0xFF);
    for (int i = 1; i >= 0; --i) result[off++] = static_cast<char>((label_id >> (i * 8)) & 0xFF);
    return result;
}
...
void ValueCodec::decodeEdgeAdjacency(std::string_view data, VertexId& src_id, VertexId& dst_id, uint64_t& seq,
    ...
    size_t off = 0;
    for (int i = 0; i < 8; ++i) src_id = (src_id << 8) | static_cast<uint8_t>(data[off++]);
    for (int i = 0; i < 8; ++i) dst_id = (dst_id << 8) | static_cast<uint8_t>(data[off++]);
    for (int i = 0; i < 8; ++i) seq = (seq << 8) | static_cast<uint8_t>(data[off++]);
    for (int i = 0; i < 2; ++i) label_id = (label_id << 8) | static_cast<uint8_t>(data[off++]);
}
