...
public:
    AsyncGraphStore(IGraphStore& store, IoScheduler& io, GraphTxnHandle txn)
        : store_(&store), io_(&io), txn_(txn) {}

    void setTransaction(GraphTxnHandle txn) { txn_ = txn; }
    ...
    folly::coro::AsyncGenerator<std::vector<VertexId>> scanVerticesByLabel(LabelId label_id) {
        ...
        auto cursor = co_await io_->dispatch(
            [this, label_id]() { return store_->createVertexScanCursor(txn_, label_id); });
    ...
    folly::coro::AsyncGenerator<std::vector<IGraphStore::EdgeIndexEntry>> scanEdges(
        VertexId vid, Direction direction, std::optional<EdgeLabelId> label_filter) {
        constexpr size_t BATCH = 1024;
        auto cursor = co_await io_->dispatch(
            [this, vid, direction, label_filter]() { return store_->createEdgeScanCursor(txn_, vid, direction, label_filter); });
    ...
    folly::coro::AsyncGenerator<std::vector<IGraphStore::EdgeTypeIndexEntry>> scanEdgesByType(
        EdgeLabelId label_id, std::optional<VertexId> src_filter, std::optional<VertexId> dst_filter) {
        constexpr size_t BATCH = 1024;
        auto cursor = co_await io_->dispatch(
            [this, label_id, src_filter, dst_filter]() { return store_->createEdgeTypeScanCursor(txn_, label_id, src_filter, dst_filter); });
    ...
    folly::coro::Task<bool> insertEdge(EdgeId eid, VertexId src_id, VertexId dst_id, EdgeLabelId label_id,
                                        uint64_t seq, const Properties& props) {
        auto result = co_await io_->dispatch(
            [this, eid, src_id, dst_id, label_id, seq, &props]() -> bool {
    ...
    IGraphStore& store() { return *store_; }
