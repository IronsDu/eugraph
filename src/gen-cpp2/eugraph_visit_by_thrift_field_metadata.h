/**
 * Autogenerated by Thrift for /home/dodo/code/fuck/eugraph/proto/eugraph.thrift
 *
 * DO NOT EDIT UNLESS YOU ARE SURE THAT YOU KNOW WHAT YOU ARE DOING
 *  @generated @nocommit
 */
#pragma once

#include <thrift/lib/cpp2/visitation/visit_by_thrift_field_metadata.h>
#include "gen-cpp2/eugraph_metadata.h"

namespace apache {
namespace thrift {
namespace detail {

template <>
struct VisitByFieldId<::eugraph::thrift::PropertyDefThrift> {
  template <typename F, typename T>
  void operator()([[maybe_unused]] F&& f, int32_t fieldId, [[maybe_unused]] T&& t) const {
    switch (fieldId) {
    case 1:
      return f(0, static_cast<T&&>(t).name_ref());
    case 2:
      return f(1, static_cast<T&&>(t).type_ref());
    case 3:
      return f(2, static_cast<T&&>(t).is_required_ref());
    default:
      throwInvalidThriftId(fieldId, "::eugraph::thrift::PropertyDefThrift");
    }
  }
};

template <>
struct VisitByFieldId<::eugraph::thrift::DateTimeValueThrift> {
  template <typename F, typename T>
  void operator()([[maybe_unused]] F&& f, int32_t fieldId, [[maybe_unused]] T&& t) const {
    switch (fieldId) {
    case 1:
      return f(0, static_cast<T&&>(t).kind_ref());
    case 2:
      return f(1, static_cast<T&&>(t).year_ref());
    case 3:
      return f(2, static_cast<T&&>(t).month_ref());
    case 4:
      return f(3, static_cast<T&&>(t).day_ref());
    case 5:
      return f(4, static_cast<T&&>(t).hour_ref());
    case 6:
      return f(5, static_cast<T&&>(t).minute_ref());
    case 7:
      return f(6, static_cast<T&&>(t).second_ref());
    case 8:
      return f(7, static_cast<T&&>(t).nanos_ref());
    case 9:
      return f(8, static_cast<T&&>(t).tz_offset_min_ref());
    case 10:
      return f(9, static_cast<T&&>(t).tz_name_ref());
    default:
      throwInvalidThriftId(fieldId, "::eugraph::thrift::DateTimeValueThrift");
    }
  }
};

template <>
struct VisitByFieldId<::eugraph::thrift::TimeValueThrift> {
  template <typename F, typename T>
  void operator()([[maybe_unused]] F&& f, int32_t fieldId, [[maybe_unused]] T&& t) const {
    switch (fieldId) {
    case 1:
      return f(0, static_cast<T&&>(t).kind_ref());
    case 2:
      return f(1, static_cast<T&&>(t).hour_ref());
    case 3:
      return f(2, static_cast<T&&>(t).minute_ref());
    case 4:
      return f(3, static_cast<T&&>(t).second_ref());
    case 5:
      return f(4, static_cast<T&&>(t).nanos_ref());
    case 6:
      return f(5, static_cast<T&&>(t).tz_offset_min_ref());
    case 7:
      return f(6, static_cast<T&&>(t).tz_name_ref());
    default:
      throwInvalidThriftId(fieldId, "::eugraph::thrift::TimeValueThrift");
    }
  }
};

template <>
struct VisitByFieldId<::eugraph::thrift::DurationValueThrift> {
  template <typename F, typename T>
  void operator()([[maybe_unused]] F&& f, int32_t fieldId, [[maybe_unused]] T&& t) const {
    switch (fieldId) {
    case 1:
      return f(0, static_cast<T&&>(t).months_ref());
    case 2:
      return f(1, static_cast<T&&>(t).days_ref());
    case 3:
      return f(2, static_cast<T&&>(t).seconds_ref());
    case 4:
      return f(3, static_cast<T&&>(t).nanos_ref());
    default:
      throwInvalidThriftId(fieldId, "::eugraph::thrift::DurationValueThrift");
    }
  }
};

template <>
struct VisitByFieldId<::eugraph::thrift::PropertyValueThrift> {
  template <typename F, typename T>
  void operator()([[maybe_unused]] F&& f, int32_t fieldId, [[maybe_unused]] T&& t) const {
    switch (fieldId) {
    case 1:
      return f(0, static_cast<T&&>(t).bool_val_ref());
    case 2:
      return f(1, static_cast<T&&>(t).int_val_ref());
    case 3:
      return f(2, static_cast<T&&>(t).double_val_ref());
    case 4:
      return f(3, static_cast<T&&>(t).string_val_ref());
    case 5:
      return f(4, static_cast<T&&>(t).int_array_ref());
    case 6:
      return f(5, static_cast<T&&>(t).double_array_ref());
    case 7:
      return f(6, static_cast<T&&>(t).string_array_ref());
    case 8:
      return f(7, static_cast<T&&>(t).datetime_val_ref());
    case 9:
      return f(8, static_cast<T&&>(t).time_val_ref());
    case 10:
      return f(9, static_cast<T&&>(t).duration_val_ref());
    case 11:
      return f(10, static_cast<T&&>(t).datetime_array_ref());
    case 12:
      return f(11, static_cast<T&&>(t).time_array_ref());
    case 13:
      return f(12, static_cast<T&&>(t).duration_array_ref());
    default:
      throwInvalidThriftId(fieldId, "::eugraph::thrift::PropertyValueThrift");
    }
  }
};

template <>
struct VisitByFieldId<::eugraph::thrift::LabelInfo> {
  template <typename F, typename T>
  void operator()([[maybe_unused]] F&& f, int32_t fieldId, [[maybe_unused]] T&& t) const {
    switch (fieldId) {
    case 1:
      return f(0, static_cast<T&&>(t).id_ref());
    case 2:
      return f(1, static_cast<T&&>(t).name_ref());
    case 3:
      return f(2, static_cast<T&&>(t).properties_ref());
    default:
      throwInvalidThriftId(fieldId, "::eugraph::thrift::LabelInfo");
    }
  }
};

template <>
struct VisitByFieldId<::eugraph::thrift::EdgeLabelInfo> {
  template <typename F, typename T>
  void operator()([[maybe_unused]] F&& f, int32_t fieldId, [[maybe_unused]] T&& t) const {
    switch (fieldId) {
    case 1:
      return f(0, static_cast<T&&>(t).id_ref());
    case 2:
      return f(1, static_cast<T&&>(t).name_ref());
    case 3:
      return f(2, static_cast<T&&>(t).properties_ref());
    case 4:
      return f(3, static_cast<T&&>(t).directed_ref());
    default:
      throwInvalidThriftId(fieldId, "::eugraph::thrift::EdgeLabelInfo");
    }
  }
};

template <>
struct VisitByFieldId<::eugraph::thrift::VertexRecord> {
  template <typename F, typename T>
  void operator()([[maybe_unused]] F&& f, int32_t fieldId, [[maybe_unused]] T&& t) const {
    switch (fieldId) {
    case 1:
      return f(0, static_cast<T&&>(t).properties_ref());
    default:
      throwInvalidThriftId(fieldId, "::eugraph::thrift::VertexRecord");
    }
  }
};

template <>
struct VisitByFieldId<::eugraph::thrift::EdgeRecord> {
  template <typename F, typename T>
  void operator()([[maybe_unused]] F&& f, int32_t fieldId, [[maybe_unused]] T&& t) const {
    switch (fieldId) {
    case 1:
      return f(0, static_cast<T&&>(t).src_vertex_id_ref());
    case 2:
      return f(1, static_cast<T&&>(t).dst_vertex_id_ref());
    case 3:
      return f(2, static_cast<T&&>(t).properties_ref());
    default:
      throwInvalidThriftId(fieldId, "::eugraph::thrift::EdgeRecord");
    }
  }
};

template <>
struct VisitByFieldId<::eugraph::thrift::BatchInsertVerticesResult> {
  template <typename F, typename T>
  void operator()([[maybe_unused]] F&& f, int32_t fieldId, [[maybe_unused]] T&& t) const {
    switch (fieldId) {
    case 1:
      return f(0, static_cast<T&&>(t).vertex_ids_ref());
    case 2:
      return f(1, static_cast<T&&>(t).count_ref());
    default:
      throwInvalidThriftId(fieldId, "::eugraph::thrift::BatchInsertVerticesResult");
    }
  }
};

template <>
struct VisitByFieldId<::eugraph::thrift::ResultValue> {
  template <typename F, typename T>
  void operator()([[maybe_unused]] F&& f, int32_t fieldId, [[maybe_unused]] T&& t) const {
    switch (fieldId) {
    case 1:
      return f(0, static_cast<T&&>(t).bool_val_ref());
    case 2:
      return f(1, static_cast<T&&>(t).int_val_ref());
    case 3:
      return f(2, static_cast<T&&>(t).double_val_ref());
    case 4:
      return f(3, static_cast<T&&>(t).string_val_ref());
    case 5:
      return f(4, static_cast<T&&>(t).vertex_json_ref());
    case 6:
      return f(5, static_cast<T&&>(t).edge_json_ref());
    case 7:
      return f(6, static_cast<T&&>(t).path_json_ref());
    case 8:
      return f(7, static_cast<T&&>(t).list_json_ref());
    case 9:
      return f(8, static_cast<T&&>(t).map_json_ref());
    default:
      throwInvalidThriftId(fieldId, "::eugraph::thrift::ResultValue");
    }
  }
};

template <>
struct VisitByFieldId<::eugraph::thrift::ResultRow> {
  template <typename F, typename T>
  void operator()([[maybe_unused]] F&& f, int32_t fieldId, [[maybe_unused]] T&& t) const {
    switch (fieldId) {
    case 1:
      return f(0, static_cast<T&&>(t).values_ref());
    default:
      throwInvalidThriftId(fieldId, "::eugraph::thrift::ResultRow");
    }
  }
};

template <>
struct VisitByFieldId<::eugraph::thrift::QueryStreamMeta> {
  template <typename F, typename T>
  void operator()([[maybe_unused]] F&& f, int32_t fieldId, [[maybe_unused]] T&& t) const {
    switch (fieldId) {
    case 1:
      return f(0, static_cast<T&&>(t).columns_ref());
    default:
      throwInvalidThriftId(fieldId, "::eugraph::thrift::QueryStreamMeta");
    }
  }
};

template <>
struct VisitByFieldId<::eugraph::thrift::ResultRowBatch> {
  template <typename F, typename T>
  void operator()([[maybe_unused]] F&& f, int32_t fieldId, [[maybe_unused]] T&& t) const {
    switch (fieldId) {
    case 1:
      return f(0, static_cast<T&&>(t).rows_ref());
    default:
      throwInvalidThriftId(fieldId, "::eugraph::thrift::ResultRowBatch");
    }
  }
};

template <>
struct VisitByFieldId<::eugraph::thrift::GraphInfo> {
  template <typename F, typename T>
  void operator()([[maybe_unused]] F&& f, int32_t fieldId, [[maybe_unused]] T&& t) const {
    switch (fieldId) {
    case 1:
      return f(0, static_cast<T&&>(t).graph_id_ref());
    case 2:
      return f(1, static_cast<T&&>(t).name_ref());
    case 3:
      return f(2, static_cast<T&&>(t).created_at_ref());
    default:
      throwInvalidThriftId(fieldId, "::eugraph::thrift::GraphInfo");
    }
  }
};
} // namespace detail
} // namespace thrift
} // namespace apache
