...
private:
    ...
    folly::coro::Task<VertexId> createNode(const std::vector<LabelId>& labels,
                                             const std::vector<std::pair<uint16_t, binder::BoundExpression>>& prop_filters,
    ...
    folly::coro::Task<std::optional<std::tuple<EdgeId, EdgeLabelId>>> findMatchingEdge(
        VertexId src_vid, VertexId dst_vid,
        const std::vector<std::pair<uint16_t, binder::BoundExpression>>& prop_filters, const DataChunk* chunk,
        size_t row_idx, VectorizedEvaluator& evaluator);

    folly::coro::Task<std::tuple<EdgeId, EdgeLabelId>> createEdge(
        VertexId src_vid, VertexId dst_vid,
        const std::vector<std::pair<uint16_t, binder::BoundExpression>>& prop_filters,
        const std::vector<std::pair<std::string, binder::BoundExpression>>& pending_props, const DataChunk* chunk,
        size_t row_idx, VectorizedEvaluator& evaluator);
    ...
    folly::coro::Task<void> registerPendingProps(
        const std::vector<std::pair<std::string, binder::BoundExpression>>& pending_props,
