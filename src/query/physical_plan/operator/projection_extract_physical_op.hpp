...
struct ColumnSpec {
    ...
    size_t source_col = 0;
    /// Logical slot id for the OUTPUT column. Set by dispatchProjectionExtract
    /// so that downstream BoundColumnRef(slot_id) can resolve via TupleSlotLayout
    /// without depending on column_index corrections.
