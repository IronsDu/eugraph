...
std::optional<BoundLogicalOperator> Binder::bindCrossWithEqualities(BoundLogicalOperator left,
    ...
    auto category = [](BoundTypeKind kind) -> int {
        switch (kind) {
    ...
    for (const auto& [name, right_info] : right_scope.symbols) {
        ...
        int left_cat = category(left_info.type.kind);
        int right_cat = category(right_info.type.kind);
        if (left_cat != right_cat || (left_cat == 0 && left_info.type.kind != right_info.type.kind)) {
            if (right_cat == 3) {
                error("VariableAlreadyBound: variable '" + name + "' already defined as " + left_info.type.toString() +
                      " but used as path");
            } else {
                error("VariableTypeConflict: variable '" + name + "' already defined as " + left_info.type.toString() +
                      " but used as " + right_info.type.toString());
            }
            return std::nullopt;
        }
