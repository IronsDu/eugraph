...
9. **删图目录不用 `remove_all`（rename 先行 + 启动兜底清理）**：`dropGraph` 过去在 `close()` 掉 WT 连接后直接 `std::filesystem::remove_all(graph_N)`。这个调用**没有进展保证**：实测在一次 TCK 里，它卡在 libstdc++ 的 `recursive_directory_iterator::__erase` → `unlinkat` 上，线程 state=R、CPU 持续增长、`/proc/<tid>/syscall` 显示 running、目录只删了一半，**持续 6.5 小时无进展**，把 dropGraph 请求、进而整个 TCK 运行拖死。且同一条路径还有第二个失败面：core dump 显示 WiredTiger 的 log-server 曾在处理这些图目录时 fail-stop panic（`__log_server` → `__wt_panic`，详见 [tests/tck-guide.md](../tests/tck-guide.md)）。
   现在的顺序是：先 `rename(graph_N, trash/graph_N_<stamp>)`——**单次系统调用、效果原子且有界**，请求路径必然推进；随后再尽力删除改名后的目录树（深度受限、每层至多两次尝试，失败只告警不留阻塞）；`GraphManager::init` 启动时扫 `trash/` 清掉上次遗留。
   残余目录只占磁盘、不影响正确性：图已从 catalog 摘除，新图用新 id 新目录。真正的删除失败最多留下一个 `trash/` 条目，等下次启动清理。
