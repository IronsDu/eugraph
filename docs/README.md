...
Shell/Client（src/program/shell/）
  → fbthrift RPC（src/program/server/，EuGraphHandler）
    → QueryExecutor（src/compute_service/executor/）：编排全流程
      ...
      → Binder（src/compute_service/planner/）：AST → BoundStatement（语义绑定，符号解析，类型推断）
      → LogicalOptimizer（src/compute_service/optimizer/）：Cascades 逻辑优化（Filter 下推等）
      → ColumnResolver（src/compute_service/planner/）：BoundVariableRef → BoundColumnRef（名称→列索引）
      → PhysicalPlanner（src/compute_service/physical_plan/）：BoundLogicalPlan → PhysicalPlan
...
| 语义绑定 | AST → `BoundStatement`（符号解析，类型推断） | `Binder`, `BoundLogicalOperator`, `ColumnResolver` | `src/compute_service/planner/` | [query/engine/query-engine-design.md] |
| 逻辑优化 | Cascades 框架逻辑优化（Filter 下推等） | `LogicalOptimizer`, `Memo`, `OptRule`, `TaskQueue` | `src/compute_service/optimizer/` | [query/optimizer/cascades-optimizer.md] |
| 物理计划 | BoundLogicalPlan → 物理算子（ID 解析、存储引用绑定、索引扫描优化） | `PhysicalPlanner`, `PhysicalOperator` | `src/compute_service/physical_plan/` | [query/engine/query-engine-design.md] |
| 查询执行器 | 协程管道执行，事务管理，向量化求值 | `QueryExecutor`, `VectorizedEvaluator` | `src/compute_service/executor/` | [query/engine/query-engine-design.md], [query/engine/execution-model.md] |
...
| 任务类型 | 先读文档 | 关键源码 |
|---------|---------|---------|
| 添加/修改 Cypher 语法 | [query/syntax/cypher-syntax.md], [query/syntax/cypher-parser-design.md], [query/engine/query-engine-design.md] | `src/compute_service/parser/`, `planner/` |
| 添加查询算子 | [query/engine/query-engine-design.md] | `src/compute_service/physical_plan/`, `executor/` |
| 添加优化规则 | [query/optimizer/cascades-optimizer.md] | `src/compute_service/optimizer/` |
| 添加/修改函数（标量/聚合） | [query/engine/query-engine-design.md] | `src/compute_service/function/`, `binder/` |
