...
Shell/Client（src/program/shell/）
  → fbthrift RPC（src/program/server/，EuGraphHandler）
    → QueryExecutor（src/compute_service/executor/）：编排全流程
      ...
      → Binder（src/compute_service/binder/）：AST → BoundLogicalPlan（语义绑定，符号解析，类型推断）
...
| 语义绑定 | AST → `BoundLogicalPlan`（符号解析，类型推断） | `Binder`, `BoundLogicalOperator` | `src/compute_service/binder/` | [query/engine/query-engine-design.md] |
| 逻辑计划 | AST → `LogicalPlan`（旧管线，保留兼容） | `LogicalPlanBuilder`, `LogicalOperator` | `src/compute_service/logical_plan/` | [query/engine/query-engine-design.md] |
