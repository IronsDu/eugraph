...
| 语义绑定 | AST → `BoundStatement`（符号解析，类型推断） | `Binder`, `BoundLogicalOperator`, `ColumnResolver` | `src/query/planner/` | [query/engine/query-engine-design.md] |
...
| 查询执行器 | 协程管道执行，事务管理，向量化求值 | `QueryExecutor`, `VectorizedEvaluator` | `src/query/executor/` | [query/engine/query-engine-design.md], [query/engine/execution-model.md] |
...
| 文档 | 说明 |
|------|------|
| [正确性测试框架设计](query/testing/correctness-testing-design.md) | TCK 提取与转换、数据驱动 GTest 跑步器架构、用例格式 |
| [正确性测试方案与计划](query/testing/cypher-correctness-test-plan.md) | 背景、TCK 能力映射、分阶段实施计划、风险分析 |
