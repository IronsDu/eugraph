...
## 9. 属性访问成本：谓词里每个属性引用各自一次 B-tree 点查

**机制（已定证，2026-09-26）**：`FilterPhysicalOp` 求值谓词时，**每个属性引用都会触发一次
独立的 `LoadVertexProp` 点查**，而投影侧的 `ProjectionExtract` 走**批量路径**（一次取回该
顶点全部属性，任意多个属性需求都被同一次读取满足）。因此成本差异只出现在**谓词**上。

全表 286,744 个 Message 的实测（引擎侧 `consume()` 口径，交替取 min）：

| 变体 | 耗时 | 每行 |
|---|---:|---:|
| `count(m)`（不读属性） | 40.4 ms | 0.14 µs |
| `count(m.creationDate)`（1 属性） | 497.0 ms | 1.73 µs |
| ＋`count(m.length)`（2 属性） | 942.9 ms | 3.29 µs |
| ＋`count(m.browserUsed)`（3 属性） | 1409.0 ms | 4.91 µs |

**每多读一个属性 ≈ +1.6 µs/行，完全线性** ⇒ 每个属性都是一次完整的 B-tree 查找
（`__wt_row_search` / `__wt_row_leaf_key` 全套重走），**没有游标复用或页内定位的摊销**。

同一 JOIN 结构下按"取值位置"分离（117k 行）：

| 变体 | 耗时 | 增量 |
|---|---:|---:|
| E 不读属性 | 261.6 ms | — |
| B 读 1 属性（`WHERE m.creationDate IS NOT NULL`） | 704.7 ms | +443 |
| A 读 3 属性（谓词 1 个 ＋ 投影 2 个） | **726.9 ms** | **仅 +22** |
| C 整顶点物化（批量取全部） | 700.6 ms | ≈B |
| **D 3 个属性都放进 WHERE** | **949.0 ms** | **比 B 再 +244** |

**结论**：
1. **投影侧多属性几乎免费**（A≈B≈C）——批量路径一次取回全部属性；
2. **谓词侧每多一个属性引用就多一次完整点查**（D−B=+244 ms）；
3. ⇒ 修法**不需要改存储布局**（此前的"顶点属性合并存储"方案已据此放弃）：只需让**谓词求值
   按顶点批量取回所需属性**（复用已有的 `LoadVertexPropCoalesce` / `batchGetVertexProperties`
   批量路径，把同一顶点的多个属性引用合并为一次读取）。

**与并发的关系（更正）**：此前记录"并发下每查询 CPU 翻倍、L1 未命中 +47.5%"是真实的，
但**不是由属性数放大**——按属性数归一后 L1 miss **率**不变（1.52% → 1.47%），
指令/cycles/L1 访问随属性数**等比**增长。即属性数与并发膨胀是两个独立效应：
前者是"多做了线性的工作量"，后者是并发下的访存延迟抬升。

**待办**：
1. 谓词属性引用合并为按顶点的一次批量读取（预期消除 D−B 的 +244 ms / 每属性 +1.6 µs/行）；
2. 顺带核对 `LoadVertexProp` 的逐行路径是否还有其它调用方（`projection_extract_physical_op.cpp:414`）；
3. 并发膨胀（§2.3.6 的 2.2×）单独立项，与属性数效应分开处理。
