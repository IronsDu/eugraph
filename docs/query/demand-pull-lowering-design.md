...
**完整流程**：见本仓库 commit history 中 `tests/test_query_executor.cpp::TckWith7Scenario1BoundEndpoint` 的 step-by-step debug 输出。

**已实施的修复方案**：方案 B'（§13.12.1）+ fresh Expand scope-aware canonical resolution。

最终采取了两层修复：
1. **方案 B'（修 Project alias slot 分配）**：`allocateSlotsInOp` 的 Project 分支采用 Pass 1（捕获 input_slot）/ Pass 2（分配 alias_slot）拆分；`collectAliasSlotMapOp` / `lowerAliasPassthroughOp` 改读 per-item `input_slot`/`alias_slot` 字段，不再从全局 `name_to_slot` 查 slot。
2. **Fresh Expand scope-aware canonical resolution**：当 MATCH 在 WITH 后重新引入同名变量（如 `WITH a AS b ... MATCH (a)-[r]->(b)`），`buildFreshExpandMap` 收集 Expand 的 planner slot，`collectPlanRequirements` 额外为其创建 PEPlan，`lowerAliasPassthrough` 通过 `findFreshExpandSlot`（只看直接 child）精确注入 RETURN 层 forward ref 的 canon。

方案 B（per-Op scope）未被采用，因为方案 B' + fresh Expand 修复以更小的改动面达到了等效的 scope 隔离效果。
