...
> - `VertexPropertyExtractPhysicalOp`：统一顶点加载算子，支持三种请求类型（LabelRequest→List\<String\> / PropRequest→scalar / VertexRequest→VertexValue），standalone 模式已实现但暂未启用（待列重写成熟）
> - `EdgePropertyExtractPhysicalOp`：边属性提取算子 ✅
> - `VertexLabelReadPhysicalOp`、`VertexPropertyReadPhysicalOp`：已删除，替换为内联实现
> - 列重写 pass、MaterializationReq 需求模型 ✅
> - 分支：`feature/lightweight-extract`，commit `2f9b77b`
> ---
>
> ## 关键设计决策：拓扑类型与语义类型分离
