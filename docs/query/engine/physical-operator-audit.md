...
> **详见**：
> - [../demand-pull-lowering-design.md](../demand-pull-lowering-design.md) — `ProjectionExtractPhysicalOp` 的具体设计、7 种 ColumnSpec、`PlanRequirements` 收集规则、`column_rewrite` 的 `BoundColumnRef` 校正等
...
**概括**：融合点/边属性抽取 + Project 语义的统一算子。详见 [../demand-pull-lowering-design.md](../demand-pull-lowering-design.md)。
...
### VarLenExpand 内部需要 labels 做 DFS（与 `../demand-pull-lowering-design.md` 协调）

当前 `VarLenExpandPhysicalOp` 在 DFS 路径构建时需要目标顶点的 labels（用于判断路径是否合法）。这部分 labels 的使用是**算子内部**的，不输出到下游列。

**解法**：VarLenExpand 内部保持当前行为——DFS 期间按需获取 labels（这是算子自身逻辑需要，不是 over-fetching）。但输出列（src、dst、path）不再携带 labels——那些由 Enricher 按需补齐。`../demand-pull-lowering-design.md` 中已经描述了这个方向。
...
> 已实现：`collectPlanRequirements` 从 `BoundLogicalPlan` 根遍历，结果存于 `PlanRequirements`（包含 `vertex_reqs` / `edge_reqs` / `path_reqs`）。`dispatchProjectionExtract` 查询此映射构建 `ColumnSpec` 数组。详见 [../demand-pull-lowering-design.md](../demand-pull-lowering-design.md)。
