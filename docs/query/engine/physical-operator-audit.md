...
> **实施状态（2026-06）**：类型系统（VertexRef / EdgeKey / PathTopology）、拓扑阶段算子输出类型、PropertyExtract 算子、列重写 pass、MaterializationReq 需求模型已实现。Enricher 算子（全量物化 VertexValue / EdgeValue / PathValue）尚未独立实现——当前通过 wrap 管线（wrapVertexLabelRead / wrapVertexPropertyRead / wrapEdgePropertyRead / wrapPathElementPropertyRead）在 RBO 路径中原地升级拓扑类型为语义类型。CBO 路径的 standalone PropertyExtractPhysicalOp 算子已编译但未接入执行管线（需 per-operator 列索引映射完全实现）。详见下文中各算子的"当前状态"标注。
>
> 分支：`feature/topology-semantic-split`，commit 范围：`dc2abbf` ~ `f93f3cb`。
>
> 涉及 47 个文件，+2954/-163 行，主要变更：
> - 类型系统：`BoundTypeKind` 新增 VERTEX_REF / EDGE_KEY / PATH_TOPOLOGY；`Value` variant 新增 VertexRef / EdgeKey / PathTopology
> - 物理算子：Scan/Expand/VarLenExpand 输出拓扑类型；新增 VertexPropertyExtractPhysicalOp / EdgePropertyExtractPhysicalOp / PathPropertyExtractPhysicalOp
> - 物理规划器：wrap 管线添加类型升级逻辑（VertexRef→Vertex, EdgeKey→Edge, PathTopology→Path）；新增 dispatchVertexPropertyExtract（未启用）
> - 优化器：MaterializationReq（need_entire / need_labels / need_props）、RequirementCollector、column_rewrite pass、Enricher 放置逻辑
> - 路径函数：nodes() / relationships() / length() 支持 PathTopology
> - 参数检查：MATCH binder 拒绝 $param 作为节点/关系谓词
>
> ---
>
> ## 关键设计决策：拓扑类型与语义类型分离
...
**现状（✅ 已改）**：输出 `VertexRef`（拓扑类型）。流式扫描，不再构造 `VertexValue`。

**当前状态**：输出类型已从 `VertexValue` 改为 `VertexRef`。标签加载由 `wrapVertexLabelRead` 按需调用 `VertexLabelReadPhysicalOp` 完成。

**待完成**：bitmap/bloom filter 增量去重替代 `unordered_set<VertexId>`；单 pass 流式扫描。

---

### 2. LabelScanPhysicalOp

**现状（✅ 已改）**：输出 `VertexRef`（拓扑类型）。已删除 `vv.labels` 的填充逻辑。标签由 `wrapVertexLabelRead` → `VertexLabelReadPhysicalOp` 按需加载。

**当前状态**：输出类型已从 `VertexValue` 改为 `VertexRef`。属性由 `wrapVertexPropertyRead` → `VertexPropertyReadPhysicalOp` 按需加载。
...
**现状（✅ 已改）**：输出 `VertexRef`（src）、`EdgeKey`（edge）、`VertexRef`（dst）。不再输出 `VertexValue` 或 `EdgeValue`。
...
- 语义升级由 wrap 管线完成：`wrapEdgePropertyRead` → `EdgePropertyReadPhysicalOp`（EdgeKey→EdgeValue），`wrapVertexLabelRead` → `VertexLabelReadPhysicalOp`（VertexRef→VertexValue），`wrapVertexPropertyRead` → `VertexPropertyReadPhysicalOp`
...
**现状（✅ 已改）**：输出 `VertexRef`（dst）、`PathValue`（path 列含 labels）、`List<EdgeValue>`（edge list 列，如有）。路径内部构建时仍加载中间顶点的 labels（DFS 需要），但 dst 列不再构造 `VertexValue`。

**当前状态**：
- dst 列：输出 `VertexRef`（原为 `VertexValue`），标签由 `wrapVertexLabelRead` 补充
...
### 5. VertexPropertyExtractPhysicalOp（✅ 已实现，未接入执行管线）

**概括**：将 `VertexRef` 列的裸顶点 ID 按需批量读属性，直接追加扁平列式 Vector 到 DataChunk。**不构造 VertexValue**。

**当前状态**：算子已实现（`vertex_property_extract_physical_op.cpp/hpp`），包含：
- 构造函数接收 `variable`、`col_idx`、`label_prop_ids`、输出 schema/types
- `executeChunk()` 实现：读取 child 的 VertexRef/VertexValue → 调用 `getVertexProperties(vid, lid)` 按 label 批量加载 → 追加扁平列
- 列类型推断：优先使用 `output_types_`，回退到运行时从第一个非空值推断
- **列追加**模式：不在原列修改，而是追加新列（改变列布局，需配合列重写使用）
- **未被启用**：当前 RBO/CBO 路径均通过 `wrapVertexPropertyRead` → `VertexPropertyReadPhysicalOp` 做原地属性加载。`dispatchVertexPropertyExtract` 函数已编写但需 `ctx.use_property_extract = true` 且 `hasPropertyExtract(chosen)` 判定才激活——当前 CBO 不产生 PropertyExtract 标签，因此此路径从不执行。

**待完成**：接入执行管线（需 per-operator 列索引映射完全实现后启用）；添加 `getVertexPropertiesBatch` 批量 API 支持。

### 5b. VertexEnricherPhysicalOp（未实现）

**概括**：将 `VertexRef` 列的裸顶点 ID 提升为 `VertexValue` 重对象，一次加载 labels + 全部属性。**仅在 `need_entire_vertex == true` 时插入**。

**理想输出**：将 `VertexRef` 列原地替换为 `VertexValue` 列（labels + properties map）。

**关键设计**：
- 只在 planner 检查到 `vertex_reqs[var].need_entire_vertex == true` 时才被插入
- 同时满足 `need_labels` 的需求（不再需要单独的 LabelRead 算子）
- 覆盖原则：优先于 VertexPropertyExtract——如果同时有 `need_entire_vertex` 和具体 `need_props`，只插此算子，Project 从 VertexValue 内部零 I/O 解引用
- 合并当前 `VertexLabelReadPhysicalOp` + `VertexPropertyReadPhysicalOp` 双算子链
- 两阶段 I/O：收集本 chunk 逻辑存活行的 vid → 一次 batch co_await → 逐行回填
- CONSTANT 保持

---

### 6. EdgePropertyExtractPhysicalOp（✅ 已实现，未接入执行管线）

**概括**：同 VertexPropertyExtractPhysicalOp 的边版本。

**当前状态**：算子已实现（`edge_property_extract_physical_op.cpp/hpp`）。读取 EdgeKey → 按需加载属性 → 追加扁平列。未被启用（同 VertexPropertyExtractPhysicalOp 的未接入原因）。

**待完成**：接入执行管线；Batch API 支持。

### 6b. EdgeEnricherPhysicalOp（未实现）

**概括**：将 `EdgeKey` 提升为 `EdgeValue` 重对象。**仅在 `need_entire_edge == true`（即 `RETURN e`）时插入**。

**理想输出**：将 `EdgeKey` 列原地替换为 `EdgeValue` 列。

**关键设计**：
- 只在 planner 检查到 `edge_reqs[var].need_entire_edge == true` 时才被插入
- 覆盖原则：优先于 EdgePropertyExtract
- 合并当前 `EdgePropertyReadPhysicalOp` + type name 解析
- 批量化 I/O，CONSTANT 保持

---

### 7. PathPropertyExtractPhysicalOp（✅ 已实现，未接入执行管线）

**概括**：将 `PathTopology` 列的路径骨架按需批量读属性，追加扁平列式 Vector。**不构造 PathValue**。

**适用场景**：需要路径中特定元素的属性（如 `RETURN [n IN nodes(p) | n.name]`），但不需要整个 PathValue。

**理想输出**：追加扁平列（如对路径中所有顶点提取 name → 追加 `List<String>` 列）。

**关键设计**：
- 展平去重批量 I/O：遍历本 chunk 存活行 → 将所有需求涉及的 vid/eid 展平到 `vector<uint64_t>` → 排序去重 → 单次 batch co_await → 按路径结构散射回填
- 覆盖原则：被 `need_entire_path` 覆盖时不插入
- 替换当前 `PathElementPropertyReadPhysicalOp`

### 7b. PathEnricherPhysicalOp（New）

**概括**：将 `PathTopology` 提升为 `PathValue`。**仅在 `need_entire_path == true`（即 `RETURN p`）时插入**。
...
### P5 — Planner 集成 🔄 部分完成（RBO wrap 管线已工作）

**涉及文件**：
- `physical_planner.cpp`：
  - `wrapVertexLabelRead` + `wrapVertexPropertyRead` → 根据 `vertex_reqs[var].need_entire_vertex` 分叉：true → `wrapVertexEnricher`；false → `wrapVertexPropertyExtract`
  - `wrapEdgePropertyRead` → 根据 `edge_reqs[var].need_entire_edge` 分叉：true → `wrapEdgeEnricher`；false → `wrapEdgePropertyExtract`
  - `wrapPathElementPropertyRead` → 根据 `path_reqs[var].need_entire_path` 分叉：true → `wrapPathEnricher`；false → `wrapPathPropertyExtract`
  - PropertyExtract / Enricher 按 per-variable 粒度插入，位于该变量首次被语义引用的位置
...
- 上述 4 个旧算子被 6 个新算子替代：`VertexPropertyExtract`、`VertexEnricher`、`EdgePropertyExtract`、`EdgeEnricher`、`PathPropertyExtract`、`PathEnricher`
- binder 扩展属性解析：强模式 `(label_id, prop_id)` 解析，减少 `BoundDynamicPropertyRef` 退化
- 提取通用 `ExpressionVisitor` 基类
- 更新 `FilterPushdownRule`：Filter 不能穿透 Enricher 或 PropertyExtract 算子
- `PhysicalOpTag` 枚举新增 `VertexPropertyExtract`、`EdgePropertyExtract`、`PathPropertyExtract` 标签
