...
> **详见**：
> - [deferred-property-loading.md](deferred-property-loading.md) — `ProjectionExtractPhysicalOp` 的具体设计、7 种 ColumnSpec、`PlanRequirements` 收集规则、`column_rewrite` 的 `BoundColumnRef` 校正等
...
**概括**：融合点/边属性抽取 + Project 语义的统一算子。详见 [deferred-property-loading.md](deferred-property-loading.md)。
...
> 已实现：`collectPlanRequirements` 从 `BoundLogicalPlan` 根遍历，结果存于 `PlanRequirements`（包含 `vertex_reqs` / `edge_reqs` / `path_reqs`）。`dispatchProjectionExtract` 查询此映射构建 `ColumnSpec` 数组。详见 [deferred-property-loading.md](deferred-property-loading.md)。
