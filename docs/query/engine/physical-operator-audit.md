...
```
拓扑阶段用：
  - VertexRef                       — 顶点引用（包装 uint64），不含任何语义数据
  - EdgeKey {id, src_id, dst_id, label_id}   — 边 ID + 拓扑信息
  - PathTopology                     — 路径骨架（vid/eid/seq 序列）

语义阶段用：
  - VertexValue                      — 含 labels、properties
  - EdgeValue                        — 含 type name、properties
  - PathValue                        — 含 labels、properties 的完整路径
```

### 提升点（Enrich Point）

提升算子（Enricher）负责 `拓扑 → 语义` 的转换。这些算子位于管道的**最晚可能位置**——紧邻第一个需要语义信息的算子之前。

注意：Enricher 不仅服务于 Project，也服务于 Filter（`WHERE n.age > 30`）、Sort（`ORDER BY n.name`）等任何引用属性/标签/type 的算子。

**Enricher 的粒度是 per-variable 的**——每个变量的 Enricher 独立放置于该变量首次被语义引用的位置，而非一个合并的大 Enricher 一次性物化所有变量。planner 从下游树为每个变量分别收集需求，不同变量的 Enricher 与 Filter 等语义算子交织排列，使得先执行的 Filter 可以削减后续 Enricher 需要处理的行数。

```
拓扑阶段                          语义阶段
  │                                  │
  AllNodeScan → uint64               │
  Expand      → uint64, EdgeKey      │
  │                                  │
  └───────────── Enricher ───────────┘
                       │
    VertexEnrich(uint64 → VertexValue, need_labels, need_props)
    EdgeEnrich(EdgeKey → EdgeValue, need_type, need_props)
    PathEnrich(PathTopology → PathValue, path_reqs)
                       │
  ─────────────────────┼──────────────────────
                       │
    Filter(WHERE n.age > 30)      ← 需要属性，在 Enricher 之后
    Project(RETURN n.name)        ← 需要属性，在 Enricher 之后
    Sort(ORDER BY n.name)         ← 需要属性，在 Enricher 之后
                       │
                  RETURN 序列化   ← 总是需要语义类型
```
...
```
VertexRequirement {
    need_labels: bool                              // 是否需要 labels 集合
    need_properties: map<LabelId, set<PropId>>     // 需要哪些 label 的哪些属性
}

EdgeRequirement {
    need_type: bool                                // 是否需要 type name
    need_properties: map<LabelId, set<PropId>>     // 需要哪些属性
}
...
需求在 planner 阶段沿算子树**从 Root 向下传播**，经过变量产出算子时实施**就地截断**。planner 在决定是否包裹、以及如何配置 Enricher 算子时，用变量名查表。
...
**现状**：扫描所有 label 的全部顶点，`unordered_set<VertexId>` 全量去重，内存堆积完成后才流式输出。每个 VertexValue 只含 `id`。

**问题**：
- 全量阻塞：下游在扫描完成前拿不到任何数据
- 内存膨胀：百万顶点图的 `unordered_set` 可达上百 MB
- 输出 `VertexValue` 只含 id，却承载了 VertexValue 的全部字段（labels、properties 空占内存）

**理想输出类型**：`DataChunk`，每行一个 `uint64` 裸顶点 ID

**理想职责**：流式扫描，每次产出一个 chunk（约 1024 行）立即返回。去重通过 bitmap 在 scan 期间增量完成，不堆积全量数据。

**关键改动**：
- 输出类型从 `VertexValue` 改为 `uint64`
- 用 bitmap / bloom filter 替代 `unordered_set<VertexId>` 做增量去重
- 改为单 pass 流式：扫一个 label 产一个 chunk，继续扫下一个 label

---

### 2. LabelScanPhysicalOp

**现状**：扫描指定 label 的顶点。对单 label 扫描直接设置 `vv.labels` 为已知集合；对多 label 扫描每行调用 `getVertexLabels(vid)`。

**问题**：
- 无条件填充 `vv.labels`，即使下游不需要
- 多 label 扫描时逐行 `co_await` 获取 labels
- 输出 `VertexValue` 即使语义数据根本未使用

**理想输出类型**：`DataChunk`，每行一个 `uint64` 裸顶点 ID

**理想职责**：只扫描指定 label 的顶点 ID。labels 由 `VertexEnrich` 按需加载。

**关键改动**：
- 输出类型从 `VertexValue` 改为 `uint64`
- 删除 `vv.labels` 的填充逻辑
- 不再需要 `getVertexLabels` 调用

---

### 3. ExpandPhysicalOp

**现状**：扫描 `(src)-[edge_label]->(dst)` 邻接表，产出 src、edge、dst 三列。EdgeValue 完整填充（id、src_id、dst_id、label_id、seq）。

**问题**：
- src/dst 以 `VertexValue{id}` 输出，但 expand 阶段不需要语义信息
- EdgeValue 总是完整填充

**理想输出类型**：
- `col[src_var]` = `uint64`（拓扑）
- `col[edge_var]` = `EdgeKey{id, src_id, dst_id, label_id}`，仅在 `edge_var_` 非空时输出
- `col[dst_var]` = `uint64`（拓扑）

**理想职责**：只做拓扑展开。不加载任何 labels、properties、type name。

**关键改动**：
- src/dst 输出改为 `uint64`
- 引入 `EdgeKey` 类型，替代 EdgeValue 的拓扑部分
- 移除 src_id 的二次提取
...
**现状**：变长路径展开。当 `path_var_` 非空时，为每条结果构建完整 PathValue——包含所有中间顶点（含 labels）、边和目标顶点。目标顶点 labels 被取两次。

**问题**：
- 路径中间顶点的 labels 无条件全部加载
- 目标顶点 labels 重复获取
- PathValue 完整构建即使下游只需要 `nodes(p)[0]`

**理想输出类型**：
- `col[src_var]` = `uint64`
- `col[dst_var]` = `uint64`
- `col[path_var]` = `PathTopology`（路径骨架——仅 vid/eid/label_id/seq 序列），仅在 `path_var_` 非空时输出

**理想职责**：只做拓扑展开，路径只含 ID 级别的骨架信息。

**关键改动**：
- 引入 `PathTopology` 类型
- PathTopology 构建时跳过 labels/properties 获取
- 移除目标顶点 labels 的重复获取

---

### 5. VertexEnrichPhysicalOp（New）

**概括**：将 `uint64` 裸 ID 提升为 `VertexValue`，按需加载 properties，极少情况才需要 labels。

**理想输出类型**：
- 常规路径：`VertexValue{id, properties}`，`need_labels = false`
- labels 路径（仅 `labels(n)` 函数或弱模式退化）：`VertexValue{id, labels, properties}`

**常规路径为何不需要 labels**：binder 在绑定属性访问时，从 MATCH 的 label filter（`n:Person`）或 CREATE 的 label 等上下文解析出 `(label_id, prop_id)`，直接走 `need_props = {Person: [name, age]}`。labels 不需要加载。

**labels 路径（边缘 case）**：仅在 binder 无法从上下文解析 label（如匿名节点）或查询显式请求 `labels(n)` 时，才设置 `need_labels = true`。

**理想职责**：一次性批量完成加载。一次 `toRows()` 修改一列，替代当前 VertexLabelRead + VertexPropertyRead 双算子链的两次拆装。两阶段 I/O：收集 chunk 内所有 vid → 一次 batch co_await → 逐行回填。

**关键设计**：
- 只在 planner 检查到 `vertex_reqs[var].need_labels == true` 或 `need_properties` 非空时才被插入
- 两阶段 I/O：收集本 chunk **逻辑存活行**的 vid（尊重上游 SelectionVector，只收集通过 Filter 的行，不扫描已被过滤的物理行）→ 一次 batch co_await → 逐行回填
- 合并当前 `VertexLabelReadPhysicalOp` + `VertexPropertyReadPhysicalOp`
- **CONSTANT 保持**：输入列为 CONSTANT 时，仅发起 1 次属性查询，输出列同样标记为 CONSTANT。禁止将 CONSTANT 展平为 1024 份重复数据

---

### 6. EdgeEnrichPhysicalOp（New）

**概括**：将 `EdgeKey` 提升为 `EdgeValue`，根据需求加载 type name 和 properties。

**理想输出类型**：`EdgeValue{id, src_id, dst_id, label_id, type?, properties?}`

**理想职责**：在语义阶段的第一站，按需补齐 edge 的语义字段。与 `VertexEnrich` 设计一致。

**关键设计**：
- 只在 planner 检查到 `edge_reqs[var].need_type == true` 或 `need_properties` 非空时才被插入
- SelectionVector 感知 + CONSTANT 保持，与 VertexEnrich 设计一致
- 批量化 I/O
- 合并当前 `EdgePropertyReadPhysicalOp` 的功能

---

### 7. PathEnrichPhysicalOp（New）

**概括**：将 `PathTopology` 提升为 `PathValue`，根据需求为路径中指定元素加载 labels 和 properties。

**理想输出类型**：`PathValue`（仅需要的元素加载了 labels/properties）

**理想职责**：替代当前的 `PathElementPropertyReadPhysicalOp`，但仅按需加载。

**关键设计**：
- 接收 `path_reqs[var]` 描述路径中哪些位置需要什么
- 没有需求时不执行任何 I/O
- 替换 `PathElementPropertyReadPhysicalOp`
- **展平去重批量 I/O**：不能对 chunk 内每条路径逐行调用 storage API。正确做法是四步——(1) 遍历本 chunk **逻辑存活行**（尊重 SelectionVector），将需求涉及的 vid/eid 展平到一个 `vector<uint64_t>`；(2) 排序去重；(3) 向 store 发起单次批量请求获取 labels/properties；(4) 按原始路径结构散射回填到 PathValue。1024 条 5-hop 路径的展平去重可将数千次单点查询压缩为一次批量调用，I/O 次数从 O(行数×路径长度) 降为 O(1)
...
binder 从 `m:Person` 解析出 `(Person, name)` 的 prop_id，直接走强模式。不需要 labels。

```
AllNodeScan(Person)  → col[0] = uint64
Expand(n, r, m)      → col[0] = uint64,  col[1] = EdgeKey,  col[2] = uint64
VertexEnrich(m,      → col[2] = VertexValue{id, properties: {name: "..."}}
  need_labels=false, need_props={Person: [name]})
Project              → col[0] = string
```

### `MATCH (n:Person)-[r:KNOWS]->(m:Person) RETURN m.name, labels(m), m.age`

binder 解析出 `(Person, name)` 和 `(Person, age)`。`labels(m)` 显式请求 labels，所以 `need_labels = true`。

```
AllNodeScan(Person)  → col[0] = uint64
Expand(n, r, m)      → col[0] = uint64,  col[1] = EdgeKey,  col[2] = uint64
VertexEnrich(m,      → col[2] = VertexValue{id, labels={Person}, properties: {name, age}}
  need_labels=true, need_props={Person: [name, age]})
...
```
AllNodeScan          → col[0] = uint64
VarLenExpand(p)      → col[0] = uint64, col[1] = uint64, col[2] = PathTopology
PathEnrich(p, {})    → col[2] = PathValue{elements enriched if needed}
...
```
AllNodeScan            → col[0] = uint64
Expand(a, r, b, KNOWS) → col[0] = uint64, col[1] = EdgeKey, col[2] = uint64
EdgeEnrich(r,          → col[1] = EdgeValue{type="KNOWS", properties: {since: 2020}}
...
这是展示 Enricher 细粒度交织的核心案例。两个变量的 Enricher 分别放在各自的 Filter 之前，n 的 Filter 先削减行数，m 的 Enricher 只处理幸存行。

```
AllNodeScan            → col[0] = uint64
Expand(n, r, m, KNOWS) → col[0] = uint64, col[1] = EdgeKey, col[2] = uint64
VertexEnrich(n,        → col[0] = VertexValue{id, properties: {age}}
  need_labels=false, need_props={Person: [age]})
Filter(n.age > 30)     → 可能削减 90% 行 —— m 的 Enricher 只处理剩余 10%
VertexEnrich(m,        → col[2] = VertexValue{id, properties: {city, name}}
  need_labels=false, need_props={Person: [city, name]})
Filter(m.city='Beijing') → 进一步削减
Project                → col[0] = string
```

**关键**：如果不做 per-variable 细粒度 Enricher，而是把 n 和 m 合并为一个大 Enricher 放在 Expand 之后，那么在 n.age > 30 过滤掉 90% 行之后，m 的属性 city/name 已经被多余加载了。per-variable 交织策略确保每个 Enricher 只处理经上游 Filter 削减后的行集。
...
VertexLabelRead → VertexPropertyRead 链式包装，每个都 `toRows()` → 改一列 → 重建 DataChunk。

**解法**：合并为 `VertexEnrichPhysicalOp`，一次拆装写 labels + properties。
...
`wrapVertexLabelRead`、`wrapVertexPropertyRead` 无条件调用。

**解法**：planner 先做需求分析，再根据 `vertex_reqs` / `edge_reqs` 决定是否包裹。
...
### P2 — 需求传递模型（PhysicalPlanner 三阶段物理优化）
...
**涉及文件**：
- 新建 `vertex_enrich_physical_op.cpp/hpp` — 合并 LabelRead + PropertyRead，两阶段 I/O
- 新建 `edge_enrich_physical_op.cpp/hpp` — 合并 EdgePropertyRead
- 新建 `path_enrich_physical_op.cpp/hpp` — 展平去重批量 I/O，替换 `PathElementPropertyReadPhysicalOp`

### P4 — Scan/Expand 输出拓扑类型
...
**涉及文件**：
- `physical_planner.cpp` — `wrapVertexLabelRead` + `wrapVertexPropertyRead` → `wrapVertexEnrich`；`wrapEdgePropertyRead` → `wrapEdgeEnrich`；`wrapPathElementPropertyRead` → `wrapPathEnrich`。Enricher 按 per-variable 粒度插入，位于该变量首次被语义引用的位置
...
**涉及文件**：
- 删除旧算子：`VertexLabelReadPhysicalOp`、`VertexPropertyReadPhysicalOp`、`EdgePropertyReadPhysicalOp`、`PathElementPropertyReadPhysicalOp`
- binder 扩展属性解析：强模式 `(label_id, prop_id)` 解析，减少 `BoundDynamicPropertyRef` 退化
- 提取通用 `ExpressionVisitor` 基类
- 更新 `FilterPushdownRule`：Filter 不能穿透 Enricher 算子
...
| 浪费点 | 是否被本文设计解决 |
|--------|------------------|
| Scan/Expand 输出胖 VertexValue | ✅ 类型分离（uint64） |
| LabelRead/PropertyRead 无条件执行 | ✅ Enricher 按需加载 |
| labels 被无意义加载 | ✅ binder 强模式解析 |
| PathElement 全量属性加载 | ✅ PathEnrich + prop_id 过滤 |
...
### VarLenExpand 内部需要 labels 做 DFS（与 `deferred-property-loading.md` 协调）

当前 `VarLenExpandPhysicalOp` 在 DFS 路径构建时需要目标顶点的 labels（用于判断路径是否合法）。这部分 labels 的使用是**算子内部**的，不输出到下游列。

**解法**：VarLenExpand 内部保持当前行为——DFS 期间按需获取 labels（这是算子自身逻辑需要，不是 over-fetching）。但输出列（src、dst、path）不再携带 labels——那些由 Enricher 按需补齐。`deferred-property-loading.md` 中已经描述了这个方向。
...
当前 `PlanContext`（`physical_planner.cpp`）持有 `label_defs`、`edge_label_defs`、`label_name_to_id`。实施需求传递后，还需要持有 `vertex_reqs`、`edge_reqs`、`path_reqs` 映射。`wrapVertexLabelRead` / `wrapVertexPropertyRead` 替换为 `wrapVertexEnrich` / `wrapEdgeEnrich` 时从此映射查询需求。
...
断言:   Filter(id(n)=10086) 在 Expand 之前或之中，n 和 m 均无 Enricher。
       m 的列类型为 VERTEX_REF（不是 VERTEX），Project 直接取 id(m)。

TC-05  MATCH (n)-[:knows]->(m) WHERE n.age > 30 RETURN id(m)
断言:   VertexEnrich(n, need_labels=false, need_props={Person:[age]}) 紧邻 Filter(n.age>30) 之前。
       m 无 Enricher（下游只要 id(m)），m 列保持 VERTEX_REF。

TC-06  MATCH (n)-[:knows]->(m) WHERE n.age > 30 AND m.city = 'Beijing' RETURN m.name
断言:   VertexEnrich(n) → Filter(n.age>30) → VertexEnrich(m) → Filter(m.city) → Project。
       两个 Enricher 独立、交织。不允许合并为一个大 Enricher。
```
...
断言:   IndexScan(id=999) 返回单行 → Expand 检测到所有输出行共享同一源行 → 源端列 a 主动提升为 CONSTANT。
       VertexEnrich(a) 识别 a 为 CONSTANT → 仅发起 1 次属性查询 → 输出列 a 保持 CONSTANT（不展平为 1024 份重复数据）。
       VertexEnrich(p) 处理密集 FLAT 列。
       断言 a 列在 Expand 后 VectorForm == CONSTANT，在 VertexEnrich 后仍为 CONSTANT。
```
...
TC-11  MATCH (a)-[:knows]->(b)-[:creates]->(c)
       ...
       RETURN c.title
断言:   算子链顺序为：
         Scan(a) → VertexEnrich(a,{gender}) → Filter(a.gender)
         → Expand(a→b) → VertexEnrich(b,{age}) → Filter(b.age)
         → Expand(b→c) → VertexEnrich(c,{type,title}) → Filter(c.type)
         → Project(c.title)
       不允许所有 Enricher 堆在 Expand 之前或之后。
```
...
断言:   返回整个顶点 → need_labels=true，need_props 覆盖所有 label 的全部属性。
       这是"正确的全量"——用户确实要整个顶点。
...
断言:   Scan → VertexEnrich(n,{age,name}) → Filter(n.age>30) → Sort(n.name) → Project。
       只有一个 Enricher（n 的需求合并为 {age, name}），放在 Filter 之前，Sort 之前已有语义类型。
```
