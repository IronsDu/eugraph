...
### 5. VertexPropertyExtractPhysicalOp（New）

**概括**：将 `VertexRef` 列的裸顶点 ID 按需批量读属性，直接追加扁平列式 Vector 到 DataChunk。**不构造 VertexValue**。

**适用场景**：只需要局部属性（`RETURN n.name, n.age`），不需要整个点对象。

**理想输出**：DataChunk 末尾追加 N 个新扁平列（N = 去重后的属性数），每列是 typed Vector（StringVector、Int64Vector、...）。

**关键设计**：
- 两阶段 I/O：收集 chunk 内所有 `VertexRef.id` → 按 (LabelId, PropId) 分组 → 一次 batch co_await → 逐行回填到新列
- 覆盖原则：被 `need_entire_vertex` 覆盖时**不插入**此算子（由 VertexEnricher 替代）
- CONSTANT 保持：输入列为 CONSTANT 时仅发起 1 次查询
- 替换当前的 `wrapVertexPropertyRead` / `VertexPropertyReadPhysicalOp`

### 5b. VertexEnricherPhysicalOp（New）
...
### 6. EdgePropertyExtractPhysicalOp（New）

**概括**：将 `EdgeKey` 列的裸边信息按需批量读属性，直接追加扁平列式 Vector。**不构造 EdgeValue**。

**适用场景**：只需要局部属性（`RETURN e.weight`），不需要整个边对象。

**理想输出**：DataChunk 末尾追加 M 个新扁平列（M = 去重后的属性数）。

**关键设计**：
- 两阶段 I/O：收集 chunk 内所有 `EdgeKey.id + label_id` → 一次 batch co_await → 逐行回填
- 覆盖原则：被 `need_entire_edge` 覆盖时不插入
- CONSTANT 保持
- 替换当前的 `wrapEdgePropertyRead` / `EdgePropertyReadPhysicalOp`

### 6b. EdgeEnricherPhysicalOp（New）
...
### 7. PathPropertyExtractPhysicalOp（New）
...
### P3 — PropertyExtract + Enricher 算子
