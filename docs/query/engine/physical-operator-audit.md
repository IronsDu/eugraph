...
> - `VertexLabelReadPhysicalOp`、`VertexPropertyReadPhysicalOp`：已删除，替换为内联实现 ✅
> - 列重写 pass、MaterializationReq 需求模型 ✅
> - 分支：`feature/lightweight-extract`
>
> ### 轻量级算子启用状态
>
> `VertexPropertyExtractPhysicalOp` 和 `EdgePropertyExtractPhysicalOp` 已完整实现，但**当前未启用**。
> 运行时仍走旧的内联加载路径（`loadVertexLabelsInPlace` / `loadVertexPropertiesInPlace`），
> 它们是已删除算子的等价内联版本，保持原地升级行为以兼容下游 `BoundPropertyRef` 求值。
>
> **已就绪的代码**：
> - `VertexPropertyExtractPhysicalOp`：三种请求类型（LabelRequest / PropRequest / VertexRequest），统一追加列模式
> - `EdgePropertyExtractPhysicalOp`：按需边属性提取
> - `dispatchVertexPropertyExtract` / `dispatchEdgePropertyExtract`：plan 分发函数
> - `canUsePropertyExtract`：安全守卫（单 scan、有标签、无动态引用、非 __anon__）
> - `PlanContext::use_property_extract`：开关标志
>
> **阻塞原因**：Extract 以**追加列**模式工作，但下游表达式（Filter/Sort/Project 中的 `BoundPropertyRef`）
> 仍通过原地加载的 `VertexValue` 列访问属性。需要列重写将 `BoundPropertyRef(n.name)` 替换为
> `BoundColumnRef(extract_col_index)` 后，下游才能正确引用 extract 产出的扁平列。
>
> **后续计划**：
> 1. 列重写支持 `BoundColumnRef` 名称匹配（`"n.name"` → extract 列索引）
> 2. 启用 `planBound` standalone 模式
> 3. `RETURN n, n.name, n.age` → 单 `VertexRequest` 优化
> 4. 扩展到 Expand dst 和 VarLenExpand dst
> 5. Thrift result format 适配新列类型
> 6. 删除剩余的旧 Read 算子（EdgePropertyReadPhysicalOp / PathElementPropertyReadPhysicalOp）
> ---
>
> ## 关键设计决策：拓扑类型与语义类型分离
...
**当前状态**：输出 `VertexRef`。标签由内联 `loadVertexLabelsInPlace`（原地升级 VertexRef→VertexValue+labels）、属性由内联 `loadVertexPropertiesInPlace`（填 properties）处理。

---

### 2. LabelScanPhysicalOp

**当前状态**：输出 `VertexRef`。同 AllNodeScan，标签和属性由内联函数处理。`VertexLabelReadPhysicalOp` 和 `VertexPropertyReadPhysicalOp` 已删除。
...
- 语义升级由内联函数完成：`loadVertexLabelsInPlace`（VertexRef→VertexValue+labels）、`loadVertexPropertiesInPlace`（填 properties）、`wrapEdgePropertyRead`（EdgeKey→EdgeValue）
...
### 5. VertexPropertyExtractPhysicalOp（✅ 已实现，standalone 模式暂未启用）

**概括**：统一顶点加载算子，支持三种请求类型：

```
LabelRequest  {source_col}              → 追加 List<String> 列（顶点 labels）
PropRequest   {source_col, lid, pid}    → 追加 typed scalar 列（单属性）
VertexRequest {source_col}              → 追加 VertexValue 列（完整点）
```

**当前状态**：
- 所有请求统一**追加新列**，不修改上游列
- 按行遍历：同 `source_col` 的请求共享 `cached_vid`，同列多属性只读一次 ID
- 上游列通过 `std::move(chunk->columns)` 直通，零拷贝
- `canUsePropertyExtract` 守卫：单 scan + 有标签 + 无 BoundDynamicPropertyRef + 仅非 __anon__ 标签
- **Standalone 模式暂未启用**（待列重写成熟后，下游表达式能正确引用新列索引）

**待完成**：接入执行管线；`RETURN n, n.name, n.age` → 单 `VertexRequest` 优化
...
### 6. EdgePropertyExtractPhysicalOp（✅ 已实现，standalone 模式暂未启用）

**概括**：边属性提取算子。读取 `EdgeKey` → 按需加载属性 → 追加扁平列。通过 `dispatchEdgePropertyExtract` + `ctx.use_property_extract` 守卫控制。
...
**涉及文件**：
- 删除旧算子：`VertexLabelReadPhysicalOp` ✅、`VertexPropertyReadPhysicalOp` ✅（已删除，内联替代）
- 上述 2 个旧算子被 `VertexPropertyExtractPhysicalOp`（三种请求类型）替代
- 待删除：`EdgePropertyReadPhysicalOp`、`PathElementPropertyReadPhysicalOp`
