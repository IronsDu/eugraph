...
| 10 | 身份 invariants 写入 AGENTS.md | 采纳（后调整） | 完整清单落在本文 4.4；AGENTS.md 只保留三条跨模块红线与指针 |
...
身份模型（Binder/DPL 强制不变量，迁移自 AGENTS.md 以避免规则漂移）：
...
5. **`all_symbols` 已删除**：禁止重新引入全局 last-write 名字索引；任何新消费者都必须走 ScopedSlotResolver。
...
- `scoped_bindings` 使用有序 `std::map<ScopeId, std::map<std::string, SlotId>>`，保证跨运行/跨进程的解析确定性。
