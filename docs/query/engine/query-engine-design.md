...
struct Column {
    BoundTypeKind type;
    VectorForm form;                 // FLAT / CONSTANT / DICTIONARY
    shared_ptr<ColumnBuffer> buffer; // FLAT：自有；DICTIONARY：与源列共享
    SelectionVector dict_sel;        // DICTIONARY：逻辑行 → 物理行
    Value constant_value;            // CONSTANT：广播值
};
```
...
struct DataChunk {
    ...
    size_t count = 0;        // 逻辑行数
    SelectionVector sel;     // 逻辑视图（identity 时 count == 逻辑行数）
...
`ColumnBuffer` / `Column` 各自提供一套按具体类型的写入接口，**不经过 `Value`**：

| 层 | 接口 |
|----|------|
| `ColumnBuffer` | `setBool / setInt64 / setDouble / setString / setVertexRef / setEdgeKey / setPathTopology / setVertexValue / setEdgeValue / setPathValue / setListValue / setMapValue` |
| `Column` | 同名转发，返回 `bool` |
...
- **多列行追加**：`DataChunk::appendRowTyped(const VertexRef* src, const VertexRef* dst, const EdgeKey* edge)` 是它的多列版本，供 `edge_index_scan`（按查询绑定了哪几个变量决定列组合）使用；参数为 `nullptr` 表示该列不写。存在理由是 `appendRow` 需要 `std::vector<Value>`：每行一次堆分配 + 1..3 次扩容，而且 `appendRow` 只有 `const&` 重载，`std::move` 进去仍是逐元素拷贝。实测该算子形状（3 列）：**50.3 ns/行、3.00 分配/行 → 7.7 ns/行、0.00 分配/行**。列与值的类型分派落在 `Column::setTypedOrValue<V>()`（复用 `setTypedImpl` 的 kind/form 保护，拒绝时回退 `setValue`），所以调用点只有一行。
