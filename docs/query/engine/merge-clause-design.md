...
- **端点自创建**：`MERGE (a:L1 {p:1})-[:R]->(b:L2 {p:2})` 在单次 MERGE 中创建整个模式（端点+关系）。TCK 目前不直接测试此场景（均通过分离的 MERGE 节点 + MERGE 关系实现），可作为后续增强。
- **多标签节点 MERGE**：`MERGE (n:L1:L2 {p:v})` 当前设计支持多标签匹配和创建，但性能优化（如复合标签索引扫描）留待后续。
