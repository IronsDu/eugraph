...
### Graph TCK 修复详情 (Phase 14)

**分支**: `feature/graph-tck-fixes`
**日期**: 2026-06-15
**结果**: 26 → **61** / 61 (+35 scenarios, +268 steps) ✅ 全部通过

#### 引擎变更

| 文件 | 变更 |
|------|------|
| `src/query/function/scalar/graph_functions.hpp` | `labelsImpl` 增加 EdgeValue 处理分支，返回边类型名 |
| `src/query/function/function_registry.cpp` | 注册 `labels(Edge)` 重载 |
| `src/query/parser/ast.hpp` | `expressionToString` 识别 `IN('Label', labels(v))` 模式格式化为 `v:Label` |
| `src/query/planner/binder/bind_expression.cpp` | 异构列表（VERTEX+EDGE 混合导致 ANY 类型合并）中预加载所有属性 |
| `src/query/physical_plan/physical_planner.cpp` | UNWIND 算子设置 EvalContext，修复 keys() 在 UNWIND 后返回空 |

#### 场景分类

| Feature | 通过 | 修复内容 |
|---------|------|---------|
| Graph1.feature | 16/16 | keys/type/labels 等函数基础用例 |
| Graph2.feature | 16/16 | 属性、标签表达式、路径序列化、UNWIND 场景 |
| Graph3.feature | 29/29 | 动态属性、类型边界、属性嵌套查询 |
...
10. ~~Graph (26→61)~~ ✅ 已全部修复（61/61）：labels(Edge) 重载、标签表达式列名、异构列表属性加载、UNWIND EvalContext
