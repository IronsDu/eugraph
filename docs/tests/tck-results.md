...
**差异明细**:
- ~~实际输出包含 `_vid` 内部字段，TCK 不期望看到~~ → 已改为 `id` 字段（TCK 自动跳过）
- ~~实际输出缺少标签名（`TheLabel`）~~ → 已在 JSON 中添加 `label` 字段
- 属性排序可能与 TCK 期望不一致

**根因**: ~~顶点序列化逻辑未按 openCypher TCK 规范输出 `(:Label {props})` 格式。~~ 已修复：server 输出 `{"id":6,"label":"TheLabel","name":"ABCDEF"}`，TCK 转换为 `(:TheLabel {name: 'ABCDEF'})`。
...
| ~~P2~~ | ~~分类4: 顶点序列化格式~~ | 广泛 | ✅ 已修复：`_vid` → `id`，添加 `label` 字段 |
