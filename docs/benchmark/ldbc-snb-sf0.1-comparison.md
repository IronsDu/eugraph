...
### 2.3.5 已落地的优化：`列 <op> 字面量` 恢复走标量内核（-6~21%）

`evalBinaryOp` 原先只在**两侧都不是临时列**时启用标量快路径。但字面量经
`acquireTempConstant` 落地后 `is_temp == true`，于是 LDBC 里最常见的一类谓词
——`WHERE m.creationDate < $maxd`——**整体退回逐行构造两个 `Value` 再比较**。
typed 内核本就有 `lhs_const`/`rhs_const` 分支（常量按第 0 行取值），判据过严。

修法：只要至少一侧是真实输入列就用输入的 selection 向量，否则维持原行为。

**A/B 对照**（同实例、同口径 `consume()`、5 轮取 min；基线 = stash 该改动后重建的 release）：

| 查询 | 基线 | 优化后 | 变化 |
|---|---:|---:|---:|
| 全表属性谓词 `WHERE m.creationDate < 1346112000000` | 598.3 ms | 559.4 ms | **-6.5%** |
| 朋友展开 + 谓词（116,958 行） | 651.1 ms | 520.1 ms | **-20%** |
| complex-9 原查询 | 1121.0 ms | 885.7 ms | **-21%** |
| 全表 `count`（与本改动无关的对照） | 45.0 ms | 41.7 ms | 持平 |

> **噪声说明**：提交后在另一时点复测，同一组查询得到 519/606/1039 ms（相对上表 ±20%）。
> 实例间与时点间波动真实存在，**引用绝对值时必须注明口径**，判断改动是否有效要看同一次
> A/B 内的相对关系（上表就是一次这样的对照）。

验证：`query_executor_tests` 573/573、`index_e2e_tests` 50/50、
全量 `ctest --preset debug` **1171/1171 通过**（含两个 bolt 集成测试与 TCK）。

**收益边界**：这只是一处"本该生效却被判据挡掉"的快路径，6–21% 不是量级变化。
complex-9 剩余大头仍是 117k 行的展开遍历（基线约 271 ms）与属性读取，
见 §2.3.4 的变体拆解与"待办"。

### 2.3.6 C2 未解决的核心：并发吞吐在 2 线程后封顶

§2.3.5 的优化只作用于单查询延迟；**并发下仍未解决**。用官方判据复核（优化后的二进制）：
连接层已干净（无 `ServiceUnavailable`）、**warmup 通过审计**（31.33 op/s），
但主阶段仍失败：
...
最慢仍是重查询：**LdbcQuery9 均值 3.9 s**（min 2.6 / max 4.6）、LdbcQuery3 2.7 s、
LdbcQuery6/12 各 ~1.0 s。

#### 量化：每查询 CPU 才是吞吐上限

用 `/proc/<pid>/stat` 的 utime+stime 增量测"每查询 CPU 消耗"（complex-9 核心，
`consume()` 口径，12–15 s/档）：

| 线程 | ops/s | 每查询 CPU | 4/8 线程理论吞吐上限 | 实际达到 |
|---:|---:|---:|---:|---:|
| 1 | 1.84 | **543 ms** | 1.84 | **100%** |
| 2 | 3.22 | **602 ms** | 3.32 | **97%** |
| 4 | 2.78 | **1114 ms** | 3.59 | 77% |
| 8 | 2.75 | **1129 ms** | 7.08 | **39%** |

两个结论：
1. **1–2 线程几乎线性**（97–100%）⇒ 引擎本身能并行；
2. **≥4 线程时每查询 CPU 翻倍**（543 → 1114–1129 ms，**2.1×**）⇒ 吞吐封顶在 ~2.8 ops/s。
   也就是说：**并发不是"排队等待"，而是每查询变贵**。

#### 已排除的解释（都实测）

| 假设 | 实测 | 结论 |
|---|---|---|
| 线程池太小 | `--compute-threads 8 --storage-io-threads 16` | 吞吐**更低**（8 线程 2.39 op/s）⇒ 排除 |
| 锁争用 | `perf` 中 mutex/futex/spin 各 **<0.2%** | 排除 |
| 缓存抖动/内存带宽 | `perf stat`：cache-miss **6.9%**（782M/11.3G）、**IPC 1.80** | 排除（命中率与 IPC 都健康） |
| 无效构造朋友顶点 | ①`count(DISTINCT f)` 7.8 ms、②`collect+size` 7.7 ms、③再加取 `f.firstName` 6.9 ms | 排除（朋友侧总成本 <10 ms，与 message 侧的 ~280 ms 差两个量级） |

#### 判断：并发下每查询 CPU 时间翻倍（单脚本复现确认）

> ⚠️ 本节成稿时我算错过一次（把**进程级总指令**当成**每查询指令**，得出"每查询多做
> 52% 工作"的错误结论）。下面的数字来自**同一脚本内**同时测吞吐与 `/proc/<pid>/stat`
> 的 CPU 时间，1/2/4 线程各 30 s，已复现两次一致：

| 线程 | 查询数 | 墙钟 | 进程 CPU 时间 | ops/s | **CPU / 查询** | 墙钟 / 查询 |
|---:|---:|---:|---:|---:|---:|---:|
| 1 | 59 | 30.1 s | 30.1 s | 1.96 | **510 ms** | 511 ms |
| 2 | 100 | 30.3 s | 58.8 s | 3.30 | **588 ms** | 303 ms |
| 4 | 87 | 30.6 s | 96.0 s | 2.84 | **1104 ms** | 352 ms |

**结论**：
1. **1→2 线程几乎线性**（ops/s 1.96 → 3.30，墙钟/查询 511 → 303 ms 减半）；
2. **4 线程吞吐反而略降**（2.84 < 3.30），而 **CPU/查询从 510 ms 涨到 1104 ms（2.2×）**；
3. 30 s 内进程消耗 96 s CPU（≈3.14 核），但只完成 87 次查询 ⇒ **吞吐上限由
   "每查询 CPU" 决定**：即使线程管够，4 / 1.104 s ≈ 3.6 ops/s 就是天花板；
4. `perf stat` 对比 1 vs 4 线程（按窗口内查询数归一）：**每查询指令几乎不变**
   （3.11 G → 3.13 G，+0.6%）、cycles +17%、IPC −13%、cache-miss/指令 +12%；
5. **每线程 CPU 分布（4 并发窗口）**：CPU 时间**只落在 4 个 compute 线程**上，
   且每个都接近满载——

   | 线程 | CPU / 20 s 墙钟 |
   |---|---:|
   | CPUThreadPool4 | 12.00 s（100%） |
   | CPUThreadPool3 | 12.00 s（100%） |
   | CPUThreadPool2 | 12.00 s（100%） |
   | CPUThreadPool1 | 11.00 s（92%） |

   ⇒ compute 线程**没有空转**，"忙等/自旋"的推测**不成立**（已否证）。

**已排除的解释**：线程池容量（8/16 更差）、锁争用（mutex 类 <0.2%）、
内存带宽/缓存抖动（cache-miss 6.9%、IPC 1.80 健康）、朋友侧顶点构造（<10 ms）。

**根因（已由硬件计数器定证）：并发下每查询的 L1 未命中与前端 stall 翻倍**

用 `perf stat` 采 1 线程与 4 线程两次运行，**按各自窗口内完成的查询数归一**：

| 指标（每查询） | 1 线程（67 次） | 4 线程（99 次） | 变化 |
|---|---:|---:|---:|
| 指令数 | 2.537 G | 2.531 G | **+0.6%（不变）** |
| cycles | 1.249 G | 1.369 G | +9.7% |
| L1-dcache 访问 | 0.947 G | 0.945 G | **−0.2%（不变）** |
| **L1-dcache 未命中** | **15.8 M** | **23.3 M** | **+47.5%** |
| dTLB 未命中 | 0.31 M | 0.33 M | +7.9% |
| **stalled-cycles-frontend** | **96.7 M** | **142.4 M** | **+47.3%** |
| branch-misses | 1.42 M | 1.84 M | +29.6% |
| IPC | 2.03 | 1.85 | −9% |

**读法**：每查询**执行的指令数、发起的 L1 访问数都不变**，但 **L1 未命中率翻倍**、
**前端 stall 翻倍** —— 即同样的工作、同样的访存次数，却因为**数据不再驻留 L1**
（4 个并发查询各自的工作集互相挤出）而 stall。IPC 下降正是这个后果。
进程 CPU 时间翻倍（510 → 1104 ms）由此得到解释。

**因此更正前几版的两个判断**：
* ~~"内存带宽/缓存抖动已排除（cache-miss 6.9%、IPC 1.80 健康）"~~ —— **错**。
  当时看的是**总体 miss 率**，没有按查询归一；按查询归一后 L1 未命中是 **+47.5%**，
  这才是并发变慢的直接证据。
* ~~"单查询协程在 compute 线程间迁移"~~ —— **实测否证**：在 `ProjectionExtract`
  与存储 `co_await` 恢复点插探针，4 并发下 91 次查询**全部 `distinct_threads=1`**，
  且每次查询只产出 1 个 chunk（chunk 边界上没有让出点）。

**已做 POC 并否证："跨池线程跳转导致 L1 失效"**（2026-09-26）

假设：计算与存储在**两个不同的线程池**上，每次存储 `co_await` 结束后 `co_viaIfAsync`
把续体调度回 compute 池 ⇒ 刚读到的数据留在 IO 线程的 L1，恢复执行的 compute 线程要吃
L1 miss。

POC 做法（`--io-on-compute-pool`）：
1. `IoScheduler::setExecutor()` 让存储派发复用**图自己的 compute pool**；
2. 关键补充：folly 的 `viaIfAsync` 是 `await_ready() { return false; }`——**无条件挂起并
   重新调度**，即使已在目标 executor 上。所以仅共用池仍会跳到**同池的另一线程**。
   再加 `setInlineDispatch(true)`：当 `co_await co_current_executor == 目标 executor` 时
   **原地执行**（不挂起、不重调度），并用计数器确认**每次派发都走了原地路径**（hop=0）。

结果（complex-9 核心，`consume()` 口径，compute=8）：

| 配置 | 1 线程 | 2 线程 | 4 线程 | 8 线程 | 4 线程 CPU/查询 |
|---|---:|---:|---:|---:|---:|
| A 基线（compute=4 / io=4） | 1.79 | 3.44 | 2.71 | 2.37 | **1141 ms** |
| B POC（共用池，仍跳转） | 1.76 | 2.77 | 2.75 | 2.26 | **1142 ms** |
| B2 POC + 原地执行（零跳转） | 1.75 | 2.98 | 2.43 | 1.97 | **1262 ms** |

**零跳转后 CPU/查询几乎不变** ⇒ **跨池跳转不是并发 CPU 翻倍的原因，该假设否证**。
（POC 改动已回退，未进入代码库；仅保留结论与复现说明。）

**结论与优化方向（按预期收益）**：
1. **缩小每次存储访问的工作集**，让热点数据留在 L1：当前每个 message 要读
   标签行 + 边 + 若干属性行（"每属性一行"布局），单行分摊的缓存足迹偏大；
   把同一顶点的属性合并为**一个 value**（一次查找取回）能同时减少查找次数与足迹；
2. **批次大小与访问顺序**：按 vid 聚集访问（提高空间局部性），避免在 117k 行上
   随机跳转；`batchGetVertexProperties` 已是批量，但游标仍在不同 B-tree 页间跳跃；
...
### 2.3.8 与 neo4j 的并发扩展性对照（同一口径）：neo4j 几乎不膨胀，我们 2.2× 膨胀

同一脚本、同一口径（`consume()` + `/proc/<pid>/stat` 增量）、同一查询与参数
（complex-9 核心，personId=32985348834013；neo4j 侧 id 传字符串、我们传整数）：

| 线程 | eugraph ops/s | eugraph CPU/查询 | 加速比 | neo4j ops/s | neo4j CPU/查询 | 加速比 |
|---:|---:|---:|---:|---:|---:|---:|
| 1 | 1.77 | **563 ms** | 1.00× | 18.15 | **62 ms** | 1.00× |
| 2 | 2.74 | 689 ms | 1.54× | 35.65 | **56 ms** | 1.96× |
| 4 | 2.51 | **1227 ms** | 1.42× | **63.51** | **62 ms** | **3.50×** |
| 8 | 2.52 | 1249 ms | 1.42× | **101.65** | **77 ms** | **5.60×** |

**读法**：
* **neo4j 的每查询 CPU 几乎不随并发变化**（62 → 77 ms，+24%），吞吐随核数接近线性
  （8 线程 5.60×），p50 仅从 54 ms 涨到 76 ms；
* **我们的每查询 CPU 翻倍**（563 → 1247 ms，+122%），吞吐 2 线程后就封顶（1.42×），
  p50 从 559 ms 涨到 2993 ms（5.4×）；
* 8 线程下绝对差距 **40×**（2.52 vs 101.65 ops/s）。

这与 §2.3.6 / known-defects §9 的计数器结论完全一致：**并发只让我们变贵，不让 neo4j 变贵**。
推测原因在**数据布局的 cache 行为**——neo4j 把关系做成定长记录、属性内联，遍历邻居是
指针追踪且每条记录只碰一次；我们每个顶点要多次 B-tree 查找 + 变长记录解码
（顶点属性"每属性一行"），并发时多个查询的工作集互相挤出 cache。

**因此优化方向聚焦于"减少每次访问触碰的记录数/字节数"**（合并顶点属性存储、改善访问局部性），
而不是调度、锁或线程池——后者已逐条实测否证（§2.3.6）。
