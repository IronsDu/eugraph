...
**当前最可能的机制（待验证）**：既然"每查询指令不变"，而并发下"每查询 CPU 时间翻倍"
且指令**没有**变多，那么同样的指令花了 2 倍时间——最可能是**单查询的协程在多个 compute
线程之间迁移**（Bolt 把 session 处理放 compute 池，算子求值里的每处 `co_await` 存储调用
都可能在**另一个** compute 线程上恢复），导致：
* 每次恢复都要换线程 → 缓存/L1 失效、线程本地状态（WiredTiger session 缓存、
  arena 分配器）反复冷启动；
* IPC 从 2.16 降到 1.87、cycles +17%，与该机制一致。

**下一步（按可验证性排序）**：
1. **验证协程迁移**：在 `PhysicalOperator` 的 chunk 边界或 `IoScheduler::dispatch` 恢复点
   打印 `std::this_thread::get_id()`，统计"同一次查询经历过多少个不同线程"；
   若 1 线程运行时为 1、4 并发时明显 >1，即证实迁移；
2. 若证实，考虑**把一次查询钉在同一个 compute 线程**上（或让 `co_await` 恢复到发起线程），
   典型手段是 per-thread 任务队列 + 亲和调度，而非全局线程池；
3. 顺带减少每查询的存储往返次数（§2.3.4 的展开 277 ms 与属性读取）；
4. `Message(creationDate)` 索引在本参数下只砍 2.8% 的行，收益有限，优先级最低。## 3. 未纳入对照的查询
