...
> **2026-09-19 修订**：complex-10 的两处**结论性内容**已更新 ——
> ① 原「瓶颈在计划开头（`VarLenExpand`），不在推导」被逐算子计时与采样**否证**；
> ② complex-10 延迟与 neo4j 倍数已重测（值深拷贝优化落地，收益 2.25–2.74×）。
> 下文带「2026-09-19 更新」标记的小节为修订处；未标记部分测得于原日期（2026-09-10），
> 其数值仍受当时 `CPU(s) scaling MHz` 影响，**不宜与新值直接相减**。
