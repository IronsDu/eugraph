...
### 2.3.4 C2 定位：成本在"遍历 + 属性谓词"，不在排序/投影

> ⚠️ **方法论比结论更重要**：顶点/属性类查询的"每行耗时"必须用**引擎侧口径**测。
> 第一版用 `list(session.run(...))` 计时，得出"顶点物化 13–37 µs/行"并据此判定它是主因——
> 那**几乎全是客户端驱动的开销**：Python 驱动把 286,744 行构造成 `Record` 要 10.2 s，
> 而服务端只需 43 ms（**238×**）。正确做法是 `session.run(q).consume()`（只排水、不建对象）。

**引擎侧实测**（`consume()`，3 轮取 min，官方参数 personId=32985348834013，
451 个 2 跳朋友、116,958 条朋友 message）：

| 变体 | 耗时 |
|---|---:|
| A 只 `count(m)`（遍历 + 计数，无属性） | 304.7 ms |
| B A ＋ `WHERE m.creationDate < $maxd` | **527.3 ms**（+223 ms ≈ +1.9 µs/行） |
| D B ＋ 物化 `id`+`creationDate` ＋ `LIMIT 20`（**无排序**） | 795.7 ms（+268 ms） |
| E B ＋ `ORDER BY cd DESC` ＋ `LIMIT 20`（**只返回 id**） | 892.7 ms |
| C 原查询（谓词 ＋ 物化 2 列 ＋ 排序 ＋ `LIMIT 20`） | **945.6 ms** |

外加两个端点参考：全表 `MATCH (m:Message) RETURN count(m)` 122 ms、
全表 `RETURN m.id` **45.8 ms**（投影比计数还快 ⇒ **读属性本身不慢**）。

**结论**：
1. **排序不是瓶颈**：D→C 只增加约 150 ms（`ORDER BY` + `LIMIT 20`，top-k 生效）；
2. 成本集中在**遍历（304 ms）＋ 属性谓词（+223 ms）＋ 物化投影（+268 ms）**；
3. `WHERE m.creationDate < $maxd` 这类**属性比较**是可直接优化的一项：每行都要
   "取值 → 构造 `Value` → 表达式求值 → 比较"，而比较本可在取到属性值时完成。

**⚠️ 实例间差异很大**：同一查询（B）在不同实例/时点测得 527 ms 与 1053 ms（2×）。
引用绝对数时必须注明实例与口径；跨实例比较请用同机交替 A/B。

**已否证的假设（都实测，避免后来人重走）**：
1. ~~"每属性一次 B-tree 查找"~~：直连 WT 微基准（`tests/tools/prop_lookup_bench.cpp`）
...
**优化落地情况**：
① **属性比较走标量内核** —— **已实施并 A/B 验证（-6~21%，见 §2.3.5）**。原判据要求两侧
   都非临时列，使 `列 <op> 字面量` 整体退回逐行构造 `Value`；
② **物化投影路径**（D 相对 B 的 +268 ms）—— 待做；
③ 顶点属性布局（一个 value 存该顶点全部属性，k 次查找降为 1 次）—— 待 ① 之后剩余量再定。
