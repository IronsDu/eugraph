...
4. 进一步用 `perf stat` 对比 1 vs 4 线程（按窗口内查询数归一）：
   **每查询指令几乎不变**（3.11 G → 3.13 G，+0.6%）、cycles +17%、IPC −13%、
   cache-miss/指令 +12% ⇒ 多出来的 CPU 时间**不是有效指令**，更像**空转/自旋/轮询**
   （类似 futex 重试、调度器 spin、跨池派发忙等）。

**已排除的解释**：线程池容量（8/16 更差）、锁争用（mutex 类 <0.2%）、
内存带宽/缓存抖动（cache-miss 6.9%、IPC 1.80 健康）、朋友侧顶点构造（<10 ms）。

**下一步（按可验证性排序）**：
1. **定位空转来源**：4 并发窗口下做 `perf` 的 per-thread 归因，重点看
   `IoScheduler` / `CPUThreadPoolExecutor` / `EventBase` 等待路径中
   `futex`、`sched_yield`、spin 类符号的占比（锁已排除，**忙等尚未排除**）；
2. 若确认跨池派发忙等，改为事件驱动唤醒而非轮询；
3. 同时评估降低单查询工作量（`Message(creationDate)` 索引——但本次参数下日期过滤
   只砍掉 2.8% 的行，收益有限）。## 3. 未纳入对照的查询
