...
修复方向：
1. 改进 `ensureTypesForQuery()` 的 regex（当前可能漏掉 `:R` 这种简写形式）
2. 或改用 AST 提取边类型名称（类似 `isQuerySupported` 的 AST 遍历方式）
3. 或让 CREATE 语句在运行时自动创建不存在的边类型
...
| **P1** | 分类4: 边类型未找到 | ~8 | ensureTypesForQuery 改进 |
...
1. **立即**: 合并当前 PR（TCK 基础设施 + graph_manager 竞态修复 + AST 精准 skip）
2. **P1-1**: 改进 `ensureTypesForQuery()` 用 AST 提取类型（解决分类4）
