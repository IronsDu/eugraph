...
- **ANY 类型属性的算术运算**：`getOrCreateAnonPropId(prop_name, PropertyType::ANY)` 注册的属性，`BoundType` 为 `ANY`。`resolveBinaryBatchFn` 原仅匹配同类型对（`INT64+INT64`、`DOUBLE+DOUBLE`），遇到 `ANY+INT64` 等组合返回 `nullptr`，结果列全为 null。现增加退路：`ANY+concrete` → concrete 侧的 batch 函数；`ANY+ANY` → `genericAddBatch` 等按运行时 Value 类型分发（支持 int64、double、混合提升、string concat）。`BoundPropertyRef.property_name` 增设（之前缺失导致 eval 按名匹配 fallback 无法触发）。
