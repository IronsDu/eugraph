...
def generate_step_diff(baseline: dict, current: dict) -> tuple[str, int]:
...
def fail_on_regression_requested() -> bool:
    """Whether a new regression should fail the run.

    Read at call time, not at configure time: CMake bakes add_test's command line when
    it configures, but CI sets TCK_FAIL_ON_REGRESSION only for the step that runs ctest.
    A configure-time check therefore never sees it.
    """
    import os
    return os.environ.get("TCK_FAIL_ON_REGRESSION", "") not in ("", "0")
...
def main():
    ...
    parser.add_argument("--fail-on-regression", action="store_true",
                        help="与 --step-baseline-file 比对出新增回归时以非零码退出；"
                             "默认关闭（本地不传基线时行为不变）。"
                             "环境变量 TCK_FAIL_ON_REGRESSION 也能打开它——"
                             "CMake 在【配置期】就把 add_test 的命令行固化了，"
                             "而 CI 是在【运行 ctest 那一步】才设这个变量，"
                             "所以必须在这里于运行期读取，否则永远读不到")
        ...
        if summary:
            ...
            if step_results_path and os.path.isfile(step_results_path):
                ...
                if current_steps and baseline_steps:
                    step_diff, regression_count = generate_step_diff(baseline_steps, current_steps)
                    if step_diff:
                        report = report + '\n' + step_diff
                        print(f"[run_tck] Step-level comparison appended", flush=True)
                    if regression_count:
                        print(f"[run_tck] {regression_count} step-level regression(s) vs baseline", flush=True)
                        if fail_on_regression_requested():
    ...
    # 场景失败本身不改变退出码（项目有已知且容忍的偏差）；
    # 只有显式要求（CI）且相对基线出现新增回归时才失败。
    if fail_on_regression:
        print("[run_tck] Failing due to TCK step regressions vs baseline", file=sys.stderr, flush=True)
        sys.exit(1)
